...
def ConnectionMatrices(SRCFull,TRGTIni,fcs,NV,NF,NVF):
    Phi=sp.lil_matrix((3*NVF*NF,3*NVF*NF))
    B=sp.lil_matrix((3*NVF*NF,3*NV))
    A=sp.lil_matrix((3*NVF*NF,3*NV))
    
    SRCIni=np.reshape(SRCFull[:,0],(NV,3))
    f=np.reshape(fcs,NVF*NF)
    SRC=np.reshape(SRCIni[f,:],(NF,3*NVF))
    TRGT=np.reshape(TRGTIni[f,:],(NF,3*NVF))
    ...
    for t in range(NF):
        ...
        Phi[3*NVF*t:3*NVF*t+3*NVF,3*NVF*t:3*NVF*t+3*NVF]=np.reshape(GetMatrices(InPut[t,:]),(3*NVF,3*NVF))
    ...
    X=(PsdA.dot(Phi.dot(A-B)+B)).dot(SRCFull)

    return X#A-B,B,PsdA 
#####################################

def PICloseForm(P,TInpt,F,NV,NPs,NF,NVF):    
...
class DTToolsPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        self.layout.operator("dt.tools",text='GetMatrices').seqType="Initialization"
